...
// add-ons grid. Shows the price alongside the label.
...
export function Checkbox({ label, checked, onChange, formattedPrice }: CheckboxProps) {
  return (
    <button
      type="button"
      role="checkbox"
      ...
      className={`hh-focus-ring group flex w-full items-center justify-between gap-3 rounded-xl border px-4 py-3 text-left transition-all duration-200 ${
        checked
    ...
    >
      ...
      <span className={`text-xs font-semibold tabular-nums ${checked ? 'text-[#E8CF83]' : 'text-slate-500'}`}>
        {formattedPrice}
      </span>
    </button>
  )
